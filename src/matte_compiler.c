...
matteTokenizer_t * matte_tokenizer_create(const uint8_t * data);
...
matteSyntaxGraphNode_t * matte_syntax_graph_node_construct(const char  *);
...
void matte_compiler_report_error
...
uint8_t * matte_compiler_run(
    ...
    matteTokenizer_t * w = matte_tokenizer_create(source);
    matteArray_t * arr = matte_array_create(sizeof(matteToken_t *));
    matteSyntaxGraph_t * st = matte_syntax_graph_create(w, arr);
    ...
    matte_syntax_graph_add_construct_path(st, MATTE_SYNTAX_CONSTRUCT_VALUE,
        ...
        matte_syntax_graph_node_split(
            ...
            matte_syntax_graph_add_construct(MATTE_SYNTAX_CONSTRUCT_EXPRESSION),
            matte_syntax_graph_add_construct(MATTE_TOKEN_OBJECT_ACCESSOR_BRACKET_END),
            matte_syntax_graph_node_end(),
            NULL
...
matteToken_t * matte_tokenizer_next(matteTokenizer_t * t, matteTokenType_t ty) {
    ...
    switch(ty) {
      case MATTE_TOKEN_LITERAL_NUMBER: {
        ...
        t->asciiBuffer[ccount] = 0;
        double f;
        if (sscanf(t->asciiBuffer, "%g", &f) == 1) {
            return new_token(
                out, //xfer ownership
                currentLine,
                currentCh,
