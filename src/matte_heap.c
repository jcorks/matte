    ...
    switch(key.binID) {
      ...
      case MATTE_VALUE_TYPE_STRING: {
        ...
        if (value) {
            ...
            *value = out;
            matte_table_insert(m->keyvalues_string, str);
        }
        return;
      }
      ...
      case MATTE_VALUE_TYPE_NUMBER: {
        ...
        if (index >= matte_array_get_size(m->keyvalues_number)) {
            ...
            for(i = currentLen; i < index; ++i) {
                matte_array_at(m->keyvalues_number, matteValue_t, i) = matte_value_new_value(key.heap);
            }
        }
...
matteValue_t matte_value_object_set(matteValue_t v, matteValue_t key, matteValue_t value) {
    if (v.binID != MATTE_VALUE_TYPE_OBJECT) {
        ...
        return matte_value_new_value(v.heap);
    }
    if (key.binID == 0) {
    ...
    if ((accessor = matte_table_find(m->keyvalues, MATTE_STR_CAST("assigner")))) {
        ...
        return object_put_prop(m, key, val);
    }
...
void matte_heap_garbage_collect(matteHeap_t * h) {
    ...
    for(matte_table_iter_start(iter, h->toSweep);
        ...
        matteObject_t * v = matte_table_iter_get_key(iter);
    ...
    for(i = 0; i < len; ++i) {
        ...
        if (m->refs == 0) {
            if (scriptFinalizer) {
                ...
                matte_vm_call(v.heap->vm, scriptFinalizer, MATTE_ARR_CAST(&object, matteValue_t, 1));
            }
