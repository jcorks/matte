MATTE_EXT_FN(matte_utility__system) {
    ...
    if (matte_array_get_size(args) < 1) {
        matte_vm_raise_error_string(vm, MATTE_VM_STR_CAST(vm, "system() requires the first argument to be a path to a file."));
...
static void matte_system__utility(matteVM_t * vm) {
    matte_vm_set_external_function(vm, MATTE_VM_STR_CAST(vm, "__matte_::utility_system"),   1, matte_utility__system, NULL);
    matte_vm_set_external_function(vm, MATTE_VM_STR_CAST(vm, "__matte_::utility_initRand"), 0, matte_utility__init_rand, NULL);
    matte_vm_set_external_function(vm, MATTE_VM_STR_CAST(vm, "__matte_::utility_nextRand"), 0, matte_utility__next_rand, NULL);
    matte_vm_set_external_function(vm, MATTE_VM_STR_CAST(vm, "__matte_::utility_exit"),     1, matte_utility__exit, NULL);
}
